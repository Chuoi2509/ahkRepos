class WinClip extends WinClip_base
{
  ...
  Copy( timeout = 1, toReturn:="text", method = 0 )
  {
    this.Snap( data )
    ...
    if( method = 1 )
      ...
      SendInput, {Ctrl Down}c{Ctrl Up} ;ctrl+c
    ClipWait,% timeout, 1
    ...
    If (toReturn="text")
      return this.GetText()
    else if (toReturn="ret")
      return !ret
  }
  
  iCopy( timeout = 1, toReturn:="text", method = 0 )
  {
    this._IsInstance( A_ThisFunc )
    this.Snap( data )
    this.Clear()    ;clearing the clipboard
  ...
  Paste( plainText = "", method = 0 )
  {
    ...
    if( method = 1 )      
      SendInput, +{Ins}       ;paste plain text only ~= ^+v
    else
      SendInput, {Ctrl Down}v{Ctrl Up} ;ctrl+v

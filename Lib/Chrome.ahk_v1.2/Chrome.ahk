...
class Chrome
{
	...
	*/
	__New(ProfilePath:="", URLs:="about:blank", Flags:="", ChromePath:="", DebugPort:="")
	{
		...
		; Verify ChromePath
		if (ChromePath == "")
			FileGetShortcut, %A_StartMenuCommon%\Programs\Cent Browser.lnk, ChromePath
		if (ChromePath == "")
	...
	*/
	class Page
	{
		...
		*/
		Evaluate(JS, centActivate:=true)
		{
			if (centActivate=true)
	...
	Jxon_Load(ByRef src, args*)	;From Jxon.ahk lib (https://github.com/cocobelgica/AutoHotkey-JSON/blob/master/Jxon.ahk)
	{
		static q := Chr(34)
	...
	Jxon_Dump(obj, indent:="", lvl:=1)  ;From Jxon.ahk lib (https://github.com/cocobelgica/AutoHotkey-JSON/blob/master/Jxon.ahk)
	{
		static q := Chr(34)

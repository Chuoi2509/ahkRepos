class Chrome
{
   ...
   */
   FindInstances()
   {
       ...
       for Item in ComObjGet("winmgmts:").ExecQuery("SELECT * FROM Win32_Process WHERE Name = 'chrome.exe'")
        if RegExMatch(Item.CommandLine, "--remote-debugging-port=(\d+)", Match)     ;fix needle to work 
   ...
   */
   class Page
   {
      ...
      */
      Evaluate(JS, centActivate:=true, async := true)  ;async edited
      {
        if (centActivate=true)
         ...
         ( LTrim Join
         {
            ...
            "awaitPromise": async ? LightJson.false : LightJson.true    
         }
         ))

...
Gui, Add, Text, x331 y+15, msTimeOut:
...
class UIA_Element extends UIA_Base {
	...
	; Wait until the element doesn't exist, with a default msTimeOut of 10000ms (10 seconds). Returns 1 if the element doesn't exist, otherwise 0.
	WaitNotExist(msTimeOut=10000) { 
		startTime := A_TickCount
		while ((exists := this.CurrentExists) && ((msTimeOut < 1) ? 1 : (A_tickCount - startTime < msTimeOut)))
			Sleep, 100
		return !exists
	}
	...
	*/
	...
	; Calls UIA_Element.FindByPath until the element is found and then returns it, with a msTimeOut of 10000ms (10 seconds). 
	WaitElementExistByPath(searchPath="", c="", msTimeOut=10000) { 
		startTime := A_TickCount
		while (!IsObject(el := this.FindByPath(searchPath, c)) && ((msTimeOut < 1) ? 1 : (A_tickCount - startTime < msTimeOut)))
			Sleep, 100
		return el
	}
	; Calls UIA_Element.FindFirstBy until the element is found and then returns it, with a msTimeOut of 10000ms (10 seconds). For explanations of the other arguments, see FindFirstBy
	WaitElementExist(expr, scope=0x4, matchMode=3, caseSensitive=True, mstimeOut=10000) { 
		startTime := A_TickCount
		while (!IsObject(el := this.FindFirstBy(expr, scope, matchMode, caseSensitive)) && ((mstimeOut < 1) ? 1 : (A_tickCount - startTime < mstimeOut)))
			Sleep, 100
		return el
	}
	; Tries to FindFirstBy the element and if it is found then waits until the element doesn't exist (using WaitNotExist()), with a msTimeOut of 10000ms (10 seconds). For explanations of the other arguments, see FindFirstBy
	WaitElementNotExist(expr, scope=0x4, matchMode=3, caseSensitive=True, msTimeOut=10000) { 
		return !IsObject(el := this.FindFirstBy(expr, scope, matchMode, caseSensitive)) || el.WaitNotExist(msTimeOut)
	}
	; Calls UIA_Element.FindFirstByName until the element is found and then returns it, with a msTimeOut of 10000ms (10 seconds)
	WaitElementExistByName(name, scope=0x4, matchMode=3, caseSensitive=True, msTimeOut=10000) {
		startTime := A_TickCount
		while (!IsObject(el := this.FindFirstByName(name, scope, matchMode, caseSensitive)) && ((msTimeOut < 1) ? 1 : (A_tickCount - startTime < msTimeOut)))
			Sleep, 100
		return el
	}
	; Calls UIA_Element.FindFirstByType until the element is found and then returns it, with a msTimeOut of 10000ms (10 seconds)
	WaitElementExistByType(controlType, scope=0x4, msTimeOut=10000) { 
		startTime := A_TickCount
		while (!IsObject(el := this.FindFirstByType(controlType, scope)) && ((msTimeOut < 1) ? 1 : (A_tickCount - startTime < msTimeOut)))
			Sleep, 100
		return el
	}
	; Calls UIA_Element.FindFirstByNameAndType until the element is found and then returns it, with a msTimeOut of 10000ms (10 seconds)
	WaitElementExistByNameAndType(name, controlType, scope=0x4, matchMode=3, caseSensitive=True, msTimeOut=10000) {
		startTime := A_TickCount
		while (!IsObject(el := this.FindFirstByNameAndType(name, controlType, scope, matchMode, caseSensitive)) && ((msTimeOut < 1) ? 1 : (A_tickCount - startTime < msTimeOut))) {
			Sleep, 100
		}
		return el
	}

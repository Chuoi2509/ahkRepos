/*
	...
	WaitTitleChange(targetTitle="", msTimeOut=-1)
		Waits the browser title to change to targetTitle (by default just waits for the title to change), msTimeOut is in milliseconds (default is indefinite waiting)
	WaitPageLoad(targetTitle="", msTimeOut=-1, sleepAfter=500, titleMatchMode=3, titleCaseSensitive=True) 
		Waits the browser page to load to targetTitle, default msTimeOut is indefinite waiting, sleepAfter additionally sleeps for 200ms after the page has loaded. 
	Back()
	...
	JSReturnThroughTitle(js, msTimeOut=500)
...
class UIA_Chrome extends UIA_Browser {
	...
	GetCurrentMainPaneElement() {              ;replace default method from UIA_Browser
		; this.BrowserElement.WaitElementExist("ControlType=Document") 
        ;Document is similar to HTML Document of active webpage=, which changes overtime anyways
        ;And document might only appear when it's visible, e.g. Cent is active
		Loop, 2 
		{
			try {
                tempEl := this.BrowserElement.FindFirstWithOptions(4, this.EditControlCondition, 2)
                if (tempEl.Name = "Find") {            ;When Find dialog is active, main pane will be lost
                    WinActivate, % "ahk_id " this.BrowserId
                    WinWaitActive, % "ahk_id " this.BrowserId,,1
                    Send, {Esc}
                } else
                    this.URLEditElement := tempEl
            }
			try {
...
class UIA_Mozilla extends UIA_Browser {
	...
	GetAlertText(closeAlert:=True, msTimeOut:=3000) {
		local
		this.GetCurrentDocumentElement()
		startTime := A_TickCount
		alertEl := this.TWT.GetNextSiblingElement(this.TWT.GetFirstChildElement(this.DocumentPanelElement))
		while (!(IsObject(dialogEl := alertEl.FindFirst("AutomationId=commonDialogWindow")) && IsObject(OKBut := dialogEl.FindFirst(this.ButtonControlCondition))) && ((A_tickCount - startTime) < msTimeOut))
			Sleep, 100
...
class UIA_Browser {
	...
	JSSetTitle(newTitle) {
		this.jsExec("document.title=""" newTitle """; void(0);")
	}
	...
	JSAlert(js, closeAlert:=True, msTimeOut:=3000) {
		this.jsExec("alert(" js ");")
		return this.GetAlertText(closeAlert, msTimeOut)
	}
	...
	JSReturnThroughClipboard(js) {
		...
		Clipboard=
		this.("copyToClipboard(" js ");function copyToClipboard(text) {const elem = document.createElement('textarea');elem.value = text;document.body.appendChild(elem);elem.select();document.execCommand('copy');document.body.removeChild(elem);}")
		ClipWait,2
	...
	JSReturnThroughTitle(js, msTimeOut:=500) {
		...
		this.jsExec("origTitle=document.title;document.title=(" js ");void(0);setTimeout(function() {document.title=origTitle;void(0);}, " msTimeOut ")")
		startTime := A_TickCount
		Loop {
			WinGetTitle, newTitle, % "ahk_id " this.BrowserId
			Sleep, 40
		} Until ((origTitle != newTitle) || (A_TickCount - startTime > msTimeOut))
	...
	JSClickElement(selector) {
        this.jsExec("document.querySelector(""" selector """).click();")
	}
	...
	GetAlertText(closeAlert:=True, msTimeOut:=3000) {
		...
		startTime := A_TickCount
		while (!(IsObject(dialogEl := this.DialogTreeWalker.GetLastChildElement(this.BrowserElement)) && IsObject(OKBut := dialogEl.FindFirst(this.ButtonControlCondition))) && ((A_tickCount - startTime) < msTimeOut))
			Sleep, 100
		try
	...
	WaitTitleChange(targetTitle:="", msTimeOut:=-1) { 
		...
		while ((((A_TickCount - startTime) < msTimeOut) || (msTimeOut = -1)) && (targetTitle ? !this.__CompareTitles(targetTitle, newTitle) : (origTitle == newTitle))) {
			Sleep, 200
	...
	; Waits the browser page to load to targetTitle, default msTimeOut is indefinite waiting, sleepAfter additionally sleeps for 200ms after the page has loaded. 
	WaitPageLoad(targetTitle:="", msTimeOut:=-1, sleepAfter:=500, titleMatchMode:="", titleCaseSensitive:=False) {
		local
		...
		startTime := A_TickCount
		while ((A_TickCount - startTime) < msTimeOut) || (msTimeOut = -1) {
			if this.BrowserType = "Mozilla"
				this.GetCurrentReloadButton()
		...
		if ((A_TickCount - startTime) < msTimeOut) || (msTimeOut = -1)
			Sleep, %sleepAfter%
	}
